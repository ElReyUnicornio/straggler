...
func _init() -> void:
	...
	var generate_button := _add_button(
		container, "Generate mesh", preload("./reload.svg"), _on_generate_pressed)
	generate_button.custom_minimum_size.x = 110

	_add_button(container, "Clear mesh", preload("./clear.svg"), _on_clear_pressed)
...
func _on_generate_pressed() -> void:
	...
	if object.has_method("update_faces"):
		object.update_faces([1,1,1,1,1,1])


func _on_clear_pressed() -> void:
	var object = get_edited_object()
	if object.has_method("update_faces"):
		object.update_faces([0,0,0,0,0,0])
